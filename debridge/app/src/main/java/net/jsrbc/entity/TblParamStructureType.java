package net.jsrbc.entity;

import android.widget.EditText;

import net.jsrbc.frame.annotation.database.object.Column;
import net.jsrbc.frame.annotation.database.object.Table;
import net.jsrbc.frame.annotation.database.restraint.PrimaryKey;

/**
 * Created by ZZZ on 2017-12-13.
 */
@Table("TBL_PARAM_STRUCTURE_TYPE")
public final class TblParamStructureType {

    @Column("ID")
    @PrimaryKey
    private String id;

    @Column("NAME")
    private String name;

    @Column("STRUCTURE_GROUP_ID")
    private String structureGroupId;

    @Column("PARENT_ID")
    private String parentId;

    @Column("NOTES")
    private String notes;

    @Column("SORT_NO")
    private int sortNo;

    private final static TblParamStructureType EMPTY = new TblParamStructureType();

    /** 返回一个空对象 */
    public static TblParamStructureType empty() {
        EMPTY.setName("");
        return EMPTY;
    }

    @Override
    public String toString() {
        return name == null ? "" : name;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getStructureGroupId() {
        return structureGroupId;
    }

    public void setStructureGroupId(String structureGroupId) {
        this.structureGroupId = structureGroupId;
    }

    public String getParentId() {
        return parentId;
    }

    public void setParentId(String parentId) {
        this.parentId = parentId;
    }

    public String getNotes() {
        return notes;
    }

    public void setNotes(String notes) {
        this.notes = notes;
    }

    public int getSortNo() {
        return sortNo;
    }

    public void setSortNo(int sortNo) {
        this.sortNo = sortNo;
    }
}
